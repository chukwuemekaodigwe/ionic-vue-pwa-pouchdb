...
export const mutations: MutationTree<IProductState>  = {
    ...
    [ProductMutationType.deleteProduct](state, Product) {
        const product = state.products.findIndex(element => element._id === Product._id)
        if (product === -1) return
    ...
    [ProductMutationType.updateProduct](state, Product) {
        const index = state.products.findIndex(element => element._id === Product._id)
        if (index === -1) return
    ...
    loadProducts(state, products:IProduct[]){
        state.products = products
    }
